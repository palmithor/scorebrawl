...
export function SubNav({ links, children, className, ...props }: SubNavProps) {
  ...
  return (
    <div className="flex items-center sm:gap-4 py-2 sticky top-12 bg-background/80 backdrop-blur-md z-20">
        ...
        <div className={cn("flex items-center", className)} {...props}>
          {links.map((link, index) => (
            <Link
              ...
              key={link.href}
              className={cn(
                "flex h-7 items-center justify-center rounded-full px-2 sm:px-4 text-center text-sm transition-colors hover:text-primary",
